...
export class RouePelleService {
  ...
  public validation(
    ...
    for (let index = 0; index < listeRoues.length; index++) {
      ...
      if (edit == true) {
        if (
          element.numPlaque == rouePelle.numPlaque &&
          rouePelle.id != element.id
        ) {
          this.errors.numPlaque = 'Ce numéro de plaqueest déjà renseignée';
          this.errors.success = false;
        }

        if (element.numero == rouePelle.numero && rouePelle.id != element.id) {
          this.errors.numero = 'Ce numéro est déjà renseigné';
          this.errors.success = false;
        }
      } else {
        if (element.numPlaque == rouePelle.numPlaque) {
          this.errors.numPlaque = 'Ce numéro de plaque est déjà renseigné';
          this.errors.success = false;
        }
    ...
    if (rouePelle.dateFab > new Date()) {
      this.errors.dateFab =
        "La date de fabrication doit etre postérieure à aujourd'hui";
      this.errors.success = false;
    }

    if (rouePelle.numero == null) {
      this.errors.numero = 'Veuillez renseigner le numéro';
      this.errors.success = false;
    }
    ...
    if (rouePelle.numPlaque == null) {
      this.errors.numPlaque = 'Veuillez renseigner un numéro de plaque';
      this.errors.success = false;
    }
